...
export class JwtStrategyPassport extends PassportStrategy(Strategy) {
  constructor(
    ...
    private usersRepository: UsersRepository, // Users 엔티티의 레포지토리를 usersRepository 변수로 선언
  ) {
    super({
      /** JWT SECRET KEY, TOKEN이 유효한지 CHECK할 때 사용 */
      secretOrKey: fs.readFileSync(path.join(__dirname, '../../../date_course/public.pem'), 'utf8'), // Public PEM
